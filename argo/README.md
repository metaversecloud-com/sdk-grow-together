...
**Secrets:** no plaintext committed. `grow-tog0-sealedsecret.yaml` holds only ciphertext
(sealed strict-scope for namespace sdk-apps-dev); the controller unseals it into
`Secret grow-tog0-secrets`, consumed via `envFrom`. Non-secret env (incl. public
`INTERACTIVE_KEY`) lives in `grow-tog0-config`.

## Environment

| Env | Service | Namespace | Host | Health |
| --- | ------- | --------- | ---- | ------ |
| `dev` | `grow-tog0` | `sdk-apps-dev` | grow-tog0-dev-topia.topia-rtsdk.com | `/api/system/health` |
